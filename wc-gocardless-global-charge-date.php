...
add_action('admin_notices', function () {
    if (!wc_gc_global_charge_date_dependencies()) {
        echo '<div class="error"><p>' .
            esc_html__(
                'WooCommerce GoCardless Global Charge Date: Dependencies not met. Make sure WooCommerce and the GoCardless Gateway plugin are active and the expected classes exist.',
                'wc-gocardless-global-charge-date',
            ) .
            '</p></div>';
    }
});
...
function wc_gc_charge_date_settings_init()
{
    ...
    register_setting('wc_gc_charge_date_group', 'wc_gocardless_charge_day', [
        'type' => 'number',
    ...
    register_setting(
        'wc_gc_charge_date_group',
        'wc_gocardless_billing_period_start_day',
        [
            'type' => 'number',
            'sanitize_callback' => 'absint',
            'default' => 28,
    ...
    add_settings_section(
        ...
        __(
            'Global GoCardless Charge Date Settings',
            'wc-gocardless-global-charge-date',
        ),
...
function wc_gc_charge_day_field_render()
{
	...
	<input type="number" name="wc_gocardless_charge_day" value="<?php echo esc_attr(
     $option,
 ); ?>" min="1" max="28" />
	<p class="description">
		<?php esc_html_e(
      'Enter a day of the month (1-28) on which payments should be charged. Note that GoCardless may shift the actual date to the next working day.',
      'wc-gocardless-global-charge-date',
  ); ?>
	</p>
	<?php
...
function wc_gc_billing_period_start_day_field_render()
{
	...
	<input type="number" name="wc_gocardless_billing_period_start_day" value="<?php echo esc_attr(
     $option,
 ); ?>" min="1" max="28" />
	<p class="description">
		<?php esc_html_e(
      'Enter the day of the month (1-28) that marks the start of the billing period. Orders placed on or before this day are considered to belong to the previous billing period.',
      'wc-gocardless-global-charge-date',
  ); ?>
	</p>
	<?php
...
function wc_gc_charge_date_options_page()
{
	...
	<div class="wrap">
		<h1><?php esc_html_e(
      'GoCardless Charge Date Settings',
      'wc-gocardless-global-charge-date',
  ); ?></h1>
...
add_filter(
    ...
    'wc_gc_charge_date_plugin_action_links',
);
function wc_gc_charge_date_plugin_action_links($links)
{
    $settings_link =
        '<a href="' .
        admin_url('admin.php?page=wc-gc-charge-date') .
        '">' .
        __('Settings', 'wc-gocardless-global-charge-date') .
        '</a>';
...
function wc_gc_modify_gocardless_payment_params($params)
{
    ...
    $billing_period_start_day = absint(
        get_option('wc_gocardless_billing_period_start_day', 28),
    );
    ...
    if ($order_day >= $billing_period_start_day) {
        $billing_period_start = DateTime::createFromFormat(
            'Y-m-d',
            $order_date->format('Y-m-') .
                sprintf('%02d', $billing_period_start_day),
        );
    } else {
    ...
    $charge_date->setDate(
        $charge_date->format('Y'),
        $charge_date->format('m'),
        $charge_day,
    );
...
add_filter(
    ...
    'wc_gc_modify_gocardless_payment_params',
    10,
    1,
);
